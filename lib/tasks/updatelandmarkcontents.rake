...
namespace :db do
  ...
  task :updaterestaurants => :environment do
    i = 0
    j = 0
    ovverride_time_constraints = false
    if (ENV["OVERRIDE_TIME_CONSTRAINTS"] and ENV["OVERRIDE_TIME_CONSTRAINTS"] == 1)
      ovverride_time_constraints = true
    end
    
    if (ENV["UPDATE_RESTAURANTS"] and ENV["UPDATE_RESTAURANTS"].to_i == 1)
      RestaurantDetail.all.each { |restaurantdetail|
        if restaurantdetail.restaurant_detail_id != nil and restaurantdetail.restaurant_detail_id != "" and (restaurantdetail.updated_at < (DateTime.now - 30) or ovverride_time_constraints)
  ...
  task :updatelocations => :environment do
    ...
    if (ENV["UPDATE_LOCATIONS"] and ENV["UPDATE_LOCATIONS"].to_i == 1)
      LocationDetail.all.each { |locationdetail|
        if locationdetail.location_detail_id != nil and locationdetail.location_detail_id != ""  and (locationdetail.updated_at < (DateTime.now - 30) or ovverride_time_constraints)
