...
#Calling Autocomplete on the condensed place-text-field
    jQuery ->        
    $('.condensed-place-text-field').autocomplete
      minLength: 3
      source: (request, response) ->
        $.ajax 
          url: window.location.protocol + "//" + window.location.host + "/locations/pick.json"
      ...
      open: ->
        $(this).removeClass(".condensed-place-text-field").addClass "ui-corner-top"
      close: ->
        $(this).removeClass("ui-corner-top").addClass "ui-corner-all"
      focus: (event, ui) ->
        event.preventDefault()
      ...
      select: (event, ui) ->
        ...
        $(this).val ui.item.label
        $(this).siblings('.condensed-place-text-field').val splitarrayvalue[0]
