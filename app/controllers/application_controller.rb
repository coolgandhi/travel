class ApplicationController < ActionController::Base
  ...
  def after_sign_in_path_for(resource)
    loc = current_author_info.author_handle.blank? ? "" : current_author_info.author_handle
    request.env['omniauth.origin'] || stored_location_for(resource) || !loc.nil? and !loc.blank? ? ("/" + loc ) : (author_page_author_info_url(current_author_info.id, :protocol => (Rails.env.production? and CONFIG[:ENABLE_HTTPS] == "yes")  ? "https": "http"))
   end
